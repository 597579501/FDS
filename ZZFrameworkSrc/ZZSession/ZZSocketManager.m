...
//    NSLog(@"message legth is :%d",[message length] + 5);
    ...
    int length = 0;
    int index = 0;
    int dataLength = strlen(sendBuffer + 4) + 4;
    ...
    {
        length =  send(CFSocketGetNative(m_socket), sendBuffer + index, dataLength - index, 0);
        index += length;
    }
}
...
//    NSLog(@"Read Stream...");
    ...
    while (_recvThreadRun)
    {
        ...
        length = recv(CFSocketGetNative(m_socket), buffer, 4,0);
//        if(recvData != nil)
//        {
//            [recvData release];
//            recvData = nil;
